...
from reid.utils.serialization import load_checkpoint, save_checkpoint, copy_state_dict,save_checkpoint_everyiter
...
def main(args):
    ...
    model.load_state_dict(checkpoint['state_dict'])
    evaluator.evaluate(test_loader, dataset.query, dataset.gallery, dataset=args.dataset)
    top1, mAP = evaluator.evaluate(test_loader, dataset.query, dataset.gallery, rerank_topk=95, dataset=args.dataset)
    exit()
 ...
 #   mAP = evaluator.evaluate(val_loader, dataset.val, dataset.val, top1=False)
    ...
    for epoch in range(0, args.epochs):
        ...
        record_epoch = int(epoch)
        save_checkpoint_everyiter(epoch, {
            'state_dict': model.state_dict()
        }, fpath=osp.join(args.logs_dir, 'checkpoint_%i.pth.tar' %record_epoch ))
